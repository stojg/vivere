package main

import (
	. "gopkg.in/check.v1"
)

type CollisionTestSuite struct{}

var _ = Suite(&CollisionTestSuite{})

func (s *CollisionTestSuite) TestNoCircleCollision(c *C) {
	collider := &Collision{}

	a := &Entity{}
	a.physics = &ParticlePhysics{}
	a.Position = &Vector3{0, 0, 0}
	a.geometry = &Circle{Position: a.Position, Radius: 5}

	b := &Entity{}
	b.physics = &ParticlePhysics{}
	b.Position = &Vector3{10, 0, 0}
	b.geometry = &Circle{Position: b.Position, Radius: 5}

	pair, hit := collider.Detect(a, b)
	c.Assert(hit, Equals, false)
	c.Assert(pair.pen, Equals, float64(0))
	c.Assert(pair.normal, DeepEquals, &Vector3{-1, 0, 0})
}

func (s *CollisionTestSuite) TestCircleCollision(c *C) {
	collider := &Collision{}

	a := &Entity{}
	a.physics = &ParticlePhysics{}
	a.Position = &Vector3{0, 0, 0}
	a.geometry = &Circle{Position: a.Position, Radius: 5}

	b := &Entity{}
	b.physics = &ParticlePhysics{}
	b.Position = &Vector3{9, 0, 0}
	b.geometry = &Circle{Position: b.Position, Radius: 5}

	pair, hit := collider.Detect(a, b)
	c.Assert(hit, Equals, true)
	c.Assert(pair.pen, Equals, float64(1))
	c.Assert(pair.normal, DeepEquals, &Vector3{-1, 0, 0})
}

func (s *CollisionTestSuite) TestCollisionResolve(c *C) {
	collider := &Collision{}

	a := &Entity{}
	a.physics = &ParticlePhysics{Velocity: &Vector3{10, 0, 0}, InvMass: 1, forces: &Vector3{}}
	a.Position = &Vector3{0, 0, 0}
	a.geometry = &Circle{Position: a.Position, Radius: 5}

	b := &Entity{}
	b.physics = &ParticlePhysics{Velocity: &Vector3{0, 0, 0}, InvMass: 1, forces: &Vector3{}}
	b.Position = &Vector3{9, 0, 0}
	b.geometry = &Circle{Position: b.Position, Radius: 5}

	pair, hit := collider.Detect(a, b)
	c.Assert(hit, Equals, true)

	pair.restitution = 1
	pair.Resolve(1)

	c.Assert(a.physics.(*ParticlePhysics).Velocity, DeepEquals, &Vector3{0, 0, 0})
	c.Assert(a.Position, DeepEquals, &Vector3{-0.5, 0})
	c.Assert(b.physics.(*ParticlePhysics).Velocity, DeepEquals, &Vector3{10, 0, 0})
	c.Assert(b.Position, DeepEquals, &Vector3{9.5, 0})
}

func (s *CollisionTestSuite) TestCollisionResolveOpposite(c *C) {
	collider := &Collision{}

	a := &Entity{}
	a.physics = &ParticlePhysics{Velocity: &Vector3{5, 0, 0}, InvMass: 1, forces: &Vector3{}}
	a.Position = &Vector3{0, 0, 0}
	a.geometry = &Circle{Position: a.Position, Radius: 5}

	b := &Entity{}
	b.physics = &ParticlePhysics{Velocity: &Vector3{-5, 0, 0}, InvMass: 1, forces: &Vector3{}}
	b.Position = &Vector3{7, 0, 0}
	b.geometry = &Circle{Position: b.Position, Radius: 5}

	pair, hit := collider.Detect(a, b)
	c.Assert(hit, Equals, true)

	pair.restitution = 1
	pair.Resolve(1)

	c.Assert(a.physics.(*ParticlePhysics).Velocity, DeepEquals, &Vector3{-5, 0, 0})
	c.Assert(a.Position, DeepEquals, &Vector3{-1.5, 0, 0})
	c.Assert(b.physics.(*ParticlePhysics).Velocity, DeepEquals, &Vector3{5, 0, 0})
	c.Assert(b.Position, DeepEquals, &Vector3{8.5, 0, 0})
}
